...
function Normalize-WeaponDef([int] $Def) {
    if ($script:KnownKnifeDefs.Contains($Def)) {
        return 42
    }
    return $Def
}
    ...
    $itemDefs.items.PSObject.Properties |
        ForEach-Object {
            ...
            if ($item.category -eq "weapon_or_knife" -and
                $item.schema_name -notmatch "^weapon_knife") {
                $def
            }
        } |
        Sort-Object -Unique
...
$knifeDefs = @(
    $itemDefs.items.PSObject.Properties |
        ForEach-Object {
            ...
            $def = [int] $item.defidx
            if ($item.category -eq "weapon_or_knife" -and
                $item.schema_name -match "^weapon_knife") {
                $def
            }
        } |
        Sort-Object -Unique
)
$script:KnownKnifeDefs = [System.Collections.Generic.HashSet[int]]::new()
foreach ($knifeDef in $knifeDefs) {
    [void] $script:KnownKnifeDefs.Add([int] $knifeDef)
}
