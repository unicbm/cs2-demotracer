...
回合 banner 只使用固定、带版本 pin 的 DTR CDN 资源，并连续 320 个 server tick
每 tick 发送一次事件。手工/debug load、加载失败、没有实际 replay bot 证据和热身阶段
不会显示。
...
为下一次成功的 manifest plan 设置双方回放角色保留优先级。每个参数是按顺序排列、
逗号分隔的 SteamID64；`1` 最优先保留，真人加入满员阵营时从 `5` 开始释放。空阵容写
`-`，`dtr_retain clear` 可清除尚未使用的设置。GUI 会把该命令紧邻放在 `dtr_go`
之前；优先级跟随 Demo 身份，在半场换边后也不会绑定到旧 entity slot。
