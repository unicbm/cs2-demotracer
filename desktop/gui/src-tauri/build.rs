...
fn validate_windows_icon(path: &std::path::Path) {
    ...
    assert!(count > 0, "Windows icon has no image entries");
    let first_width = if bytes[6] == 0 {
        256
    } else {
        u16::from(bytes[6])
    };
    let first_height = if bytes[7] == 0 {
        256
    } else {
        u16::from(bytes[7])
    };
    assert_eq!(
        ...
        (256, 256),
        "Windows icon must keep its 256x256 frame first for Tauri high-DPI window icons"
    );
